...
class Submission:
    ...
    def __init__(self, path, style_weighting=10., *,
        ...
        self.globals = {}
        self.compiler = Compiler(self.reference + '.py',
                                 self.globals)
        self.feedback = {}
    ...
    def score(self):
        if not hasattr(self, '_score'):
            self._score = round(self._get_test_score() +
                                self._get_style_score())
        return self._score

    def _get_style_score(self):
        if 'style' not in self.feedback:
            return 0
        else:
    ...
    def compile(self):
        ...
        """
        self.compiler.compile_source(self.source)
        self.add_feedback('compilation',
                          self.compiler.report.getvalue())
